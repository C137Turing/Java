...
function addMessage(messageShowDiv, message) {
    ...
    let selfUsername= document.querySelector('.user[user-id]').getAttribute('user-id');;
    if (selfUsername == message.fromId) {
        // 消息是自己发的. 靠右
    ...
    messageDiv.innerHTML = '<div class="box">' 
        ...
        + '<p>' + message.content + '</p>'
        +' <span class="message-time">'+message.postTime+'</span>'
        + '</div>';
...
function createSession(friendId, sessionLi) {
    $.ajax({
        ...
        success: function(ret) {
            console.log("会话创建成功! sessionId = " + ret.sessionId);
